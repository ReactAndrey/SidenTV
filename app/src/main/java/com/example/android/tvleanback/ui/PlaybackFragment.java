...
public class PlaybackFragment extends VideoSupportFragment {
    ...
    private static final int UPDATE_DELAY = 1;//16;
    ...
    private void initializePlayer() {
        ...
        String streamUrl = mVideo.videoUrl;
...
//        mTrackSelector = new DefaultTrackSelector(videoTrackSelectionFactory);
    ...
    private void prepareMediaForPlaying(Uri mediaSourceUri) {
        ...
        bandwidthMeter = new DefaultBandwidthMeter();
        mediaDataSourceFactory = new DefaultDataSourceFactory(getActivity(), Util.getUserAgent(getActivity(), "mediaPlayerSample"), (TransferListener<? super DataSource>) bandwidthMeter);
        if(mediaSourceUri.toString().endsWith(".m3u8"))
            mediaSource = new HlsMediaSource.Factory(mediaDataSourceFactory).createMediaSource(mediaSourceUri);
        else
            mediaSource = new ExtractorMediaSource.Factory(mediaDataSourceFactory).createMediaSource(mediaSourceUri);
//        if(mediaSourceUri.toString().endsWith(".m3u8"))
//            mediaSource = new HlsMediaSource(
//                    mediaSourceUri,
//                    new HlsDataSourceFactory(getActivity(), userAgent),
//                    new HlsExtractorFactory(),
//                    null,
//                    null);
//        else
